document.addEventListener('DOMContentLoaded', () => {
    ...
    menuItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            if (pageId === 'in-farmacia') requestWakeLock();
    ...
    addUserForm.addEventListener('submit', (e) => {
        ...
        if (!cfRegex.test(codiceFiscaleInput)) {
            alert('Il Codice Fiscale non è in un formato valido.\nIl formato corretto è: 6 lettere, 2 numeri, 1 lettera, 2 numeri, 1 lettera, 3 numeri, 1 lettera.');
            return;
        }
    ...
    function renderNreList() {
        ...
        if (user && user.nre) {
            user.nre.forEach((nre, index) => {
                ...
                const displayText = nre.suspended
                    ? `${nre.date} - ${nre.code} - SOSP.`
                    : `${nre.date} - ${nre.code}`;

                nreDiv.innerHTML = `
                    <span>${displayText}</span>
                    <div class="nre-buttons">
    ...
    function renderFarmaciaPage() {
        ...
        if (user) {
            ...
            `;
            try {
                JsBarcode("#codice-fiscale-barcode", user.codiceFiscale, { format: "CODE39", height: 52, displayValue: true, fontSize: 16 });
            } catch (e) { console.error("Errore generazione barcode CF:", e); }
    ...
    function renderRicetta() {
        ...
        if (activeRicette.length > 0) {
            ...
            ricetteContainer.innerHTML = `
                <p class="info-ricetta info-ricetta-main">${ricettaCountText}</p>
            ...
            } catch (e) { console.error("Errore generazione barcode NRE:", e); }
        } else {
            ricetteContainer.innerHTML = '<p class="info-ricetta info-ricetta-main">NESSUNA RICETTA DA VISUALIZZARE</p>';
        }
    }
