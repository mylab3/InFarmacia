document.addEventListener('DOMContentLoaded', () => {
    ...
    menuItems.forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            // RISOLUZIONE BUG: Ricarica i dati della pagina ogni volta che viene visualizzata
            ...
            if (pageId === 'promemoria') {
                renderNreList();
            }
            if (pageId === 'impostazioni') {
                renderUsers();
            }
        });
    });
    ...
    function renderUsers() {
        ...
        users.forEach(user => {
            ...
            // questo è originaleuserDiv.innerHTML = `<p>${user.nome} ${user.cognome} - ${user.codiceFiscale}</p><button class="btn-red" onclick="deleteUser(${user.id})">Elimina</button>`;
            userDiv.innerHTML = `<p>${user.nome} ${user.cognome} - ${user.codiceFiscale}</p><div><button class="btn-red" onclick="deleteUser(${user.id})">Elimina</button></div>
`;
    ...
    function renderFarmaciaPage() {
        ...
        if (user) {
            assistitoInfo.innerHTML = `<div class="assistito-name">${user.cognome} ${user.nome}</div><div class="barcode-container"><svg id="codice-fiscale-barcode" class="barcode"></svg></div>`;
            try { JsBarcode("#codice-fiscale-barcode", user.codiceFiscale, { format: "CODE39", height: 52, displayValue: true, fontSize: 16 }); } catch (e) { console.error("Errore barcode CF:", e); }
    ...
    function renderRicetta() {
        ...
        if (activeRicette.length > 0) {
            ...
            ricetteContainer.innerHTML = `<div class="ricetta-info-header"><p class="info-ricetta info-ricetta-main">${ricettaCountText}</p><p class="info-ricetta">${ricettaIndexText}</p></div><div class="barcode-container"><svg id="nre-barcode-1" class="barcode"></svg></div><div class="barcode-container"><svg id="nre-barcode-2" class="barcode"></svg></div>`;
            try {
    ...
    function saveAndRender() {
        ...
        // Aggiorna la vista corrente per riflettere immediatamente le modifiche
